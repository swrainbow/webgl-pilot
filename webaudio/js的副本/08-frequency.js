...
function mousePressed() {
  if (!audioContext) {
    ...
    audio.src = "audio/Russ-Psycho (Pt. 2) (Explicit).mp3";
...
function draw() {
  ...
  if (audioContext) {
    ...
    stroke("#E84420");
    const drum = audioSignal(analyserNode, frequencyData, 200, 600);
    circle(cx, cy, radius * drum);

    // draw the higher frequency signal
    stroke("#ffffff");
    const voice = audioSignal(analyserNode, frequencyData, 10, 100);
